...
window.ContentTablePage = function() {
  ...
  const filteredAndSortedContents = React.useMemo(() => {
    return accountContents.filter(item => {
      ...
      // Sorting by date: break ties on the same day using the upload time
      if (sortBy === "uploadDate") {
        const stampA = `${a.uploadDate || ""} ${a.uploadTime || "00:00"}`;
        const stampB = `${b.uploadDate || ""} ${b.uploadTime || "00:00"}`;
        return sortOrder === "asc" ? stampA.localeCompare(stampB) : stampB.localeCompare(stampA);
      }
